            ...
            <div class="col-12">
                <div class="card my-4">
                    <div class="card-header">
                        <div
                            class="bg-gradient-white shadow-white  border-radius-lg pt-4 pb-3 px-3 d-flex justify-content-between align-items-center">
                            ...
                            <table class="table align-items-center table-hover mb-0">
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-secondary text-xs font-weight-bolder ps-3">Type de
                                            véhicule</th>
                                        ...
                                        <tr>
                                            ...
                                            <td class="ps-3 align-middle">
                                                @if ($typeVehicule->photo)
                                                    <img src="{{ asset('storage/' . $typeVehicule->photo) }}"
                                                        alt="{{ $typeVehicule->nom_type }}"
                                            ...
                                            <td class="text-center align-middle">
                                                <span
                                                    class="text-xs font-weight-bold">{{ $typeVehicule->kilo_initiale }}</span>
                                            </td>

                                            <td class="text-center align-middle">
                                                <span
                                                    class="text-xs font-weight-bold">{{ $typeVehicule->kilo_final }}</span>
                                            </td>

                                            <td class="text-center align-middle">
                                                <span
                                                    class="text-xs font-weight-bold">{{ $typeVehicule->tarif->kilo_tarif }}
                                                    {{ $typeVehicule->tarif->type }} /
                                                    {{ number_format($typeVehicule->tarif->prix, 2) }}
                                                    {{ $typeVehicule->tarif->devise }}</span>
                                            </td>

                                            <td class="text-center align-middle">
                                                <span
                                                    class="text-xs font-weight-bold">{{ $typeVehicule->created_at->format('d/m/Y H:i') }}</span>
                                            </td>
                                            ...
                                            {{ $typeVehicule->tarif->id }})">
                                                ...
                                                <form action="{{ route('typeVehicule.destroy', $typeVehicule->id) }}"
            ...
            <div class="modal-content shadow">
                ...
                <div class="modal-body">
                    <form id="typeVehiculeForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            <label for="modal_kilo_initiale" class="form-label"> initial</label>
                        ...
                        <div class="mb-3">
                            <label for="modal_kilo_final" class="form-label"> final</label>
                            ...
                            <select name="tarif_id" id="modal_tarif_id" class="form-select border border-secondary"
                                ...
                                @foreach ($tarifs as $tarif)
                                    <option value="{{ $tarif->id }}"
                                        data-display="{{ $tarif->kilo_tarif }}  {{ $tarif->type }} /  {{ $tarif->devise }}{{ $tarif->prix }}">
                                        {{ $tarif->kilo_tarif }}  {{ $tarif->type }} /  {{ $tarif->devise }} {{ $tarif->prix }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
